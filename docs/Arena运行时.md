...
2. **逐阶段执行**，每阶段开始时先读配置指定的角色文件分支与 profile，
   ...
   |---|---|---|
   | 00 简报 | 主控 | `00_简报.md` |
   | 01 精读 Gate+样本选择+主题深读+检索+方向 | story-architect（副本职责·策划）+ 并行多路搜索（≥8，单批并发） | `01_主题深读_检索与剧情方向.md` → **用户选定方向** |
...
- **审核 = 生成完再次查看 + 搜索网页**（`04b` 复读 + `04c` 联网对标），不是跑脚本删改文章。
  `04b`：完整复读 + 听感三问（≤3 处）+ 口播规格核对（篇幅档 / 均行长 / 空行数 / 气口行）；
  `04c`：真跑搜索，查同题近 30 天爆款带位、热梗时效与出处、外部事实，结果写进审核报告「联网对标」节，
  **未写 = 审核未完成**。
- **不再提供读感扫描脚本。** 曾加过 `scripts/fuben_craft_scan.py`（只读、永不 BLOCK），已删除：
  口播稿不做「去 AI 味」清零，用正则找文体问题的路子本身就走偏了——它找不到开头追看、笑点引擎、
  蓄势兑现、共情通道这四项，而这四项正是用户点名要的。质地只能靠精读原文与复读。
- 机检的边界：`fuben_run.py` 查日期、算术、事实锁、账号契约与输入完整性，
  **不查钩子、笑点、共情、侧面、剧情**。
