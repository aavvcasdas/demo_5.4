...
> **2026-09-23**：这两个文件曾在沙箱回滚中丢失（`作品/80/审核报告.md` 的「环境事故披露」有记录），
> 而 profile 与 SKILL.md 又硬性要求「两者都不可用就停止正文产出」——结果实际发生的是静默兜底。
> 现已按本说明与 79/80 报告里的历史版本号（v1.1.0 → v1.3.0）重建为 **v1.4.0**，
> 并在两处都写明：**读不到配置时报告 `BLOCKED: runtime config missing` 并给出缺失路径，不得直接代写正文。**
...
2. **逐阶段执行**，每阶段开始时先读配置指定的角色文件分支与 profile，
   ...
   |---|---|---|
   | 00 简报 | 主控 | `00_简报.md` |
   | 01 样本选择+主题深读+检索+方向 | story-architect（副本职责·策划）+ 并行多路搜索（≥8，单批并发） | `01_主题深读_检索与剧情方向.md` → **用户选定方向** |
  ...
  | 05b 读感扫描（可选，只读） | `scripts/fuben_craft_scan.py` | `05b_读感扫描.txt` |

3. **调用证据**不是话术而是文件：每个阶段产物真实存在、内容对应其职责、
   阶段 01 必须含**同型样本清单 + 借用的机制名**（`拆文库/NN` 编号 + 该篇 `写作手法.md` 里的手法名，
   写不出机制名就是没读）、主题深读、一次并行多批的真实搜索记录（检索词、来源、借用的质感）
   和**分类型剧情方向清单 + 用户选定结果**（每个方向写明用哪种开场模板、哪种笑点引擎、
   加码那一拍是什么）——用户裁决：不调用搜索即任务失败；未经选定方向不得动笔。
   **选样本是策划的第一个动作，排在搜索之前**：见
   [fuben-craft/样本索引.md](../skills/story-short-write/references/fuben-craft/样本索引.md)。
...
- `fuben_craft_scan.py` 是只读扫描：不打分、不判好看、永不 BLOCK、不写审核结论。
  它的已知盲区写在输出末尾——**开头追看、笑点引擎、蓄势兑现、共情通道四项没有任何正则能判，必须人读。**
- 审稿禁止按 `_archived/人生副本实录_v1_归档.md` 的旧配额验收（实证见
  [skill诊断与优化_2026-09-23.md](skill诊断与优化_2026-09-23.md) 病因 2）。
