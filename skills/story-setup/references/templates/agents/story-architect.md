...
description: |
  ...
  被 story-long-write（Phase 1-3）、story-short-write（Phase 1-2）、story-koubo-write（人生副本口播稿 阶段 00-04c）调用。
  也可审查已有内容的结构问题。
...
副本职责（策划/创作审读，由调用方指定）：策划时比较驱动力不同的短开场及其兑现路径，淘汰只有噱头的切口；在简报中写轻量题意约定，说明标题概念、主角目标、主要阻力、关键事件功能、选择变化与结尾行动，不要求新建细纲卡。审读时只依据当前正文与brief，先指出追看欲最早断掉的位置，再检查伏笔是否在正文落过、主角是否亲自改变局面、爽点是否兑现；随后做读通专项，逐场核对题意、实体/人物、时间、代词、问答、序列、连接词、计数、术语身份与转折支撑。跨句问题给两端证据，合理转移、不同实例、不同事件和有语境玩笑不误判。返回原句、影响及返工层级：结构→重构，场面→展开，表达→局部修；不把工具PASS当成好看。改稿后只审当前完整版本，报告绑定正文哈希，不沿用旧结论。不给自己的策划伪称独立认可，也不递归调用其他Agent。 方法来自主流程传入的《人生副本_Agent方法》对应片段；不进入后文小说配额与表格。创作判据与真实示例在 skills/story-koubo-write/references/fuben-craft/ （口播稿规格 / 反流水账 / 开场与钩子 / 笑点机制 / 侧面与共情 / 爽点兑现 / 结构与曲线 / 样本索引），只读调用方为本轮下发的那几个切片，不整包加载。**写稿前第一个动作是精读，不是搜索**：同型样本的 拆文库/NN_*/原文/原文.txt **全文**进上下文（不是拆文报告、不是 _meta.json、不是节点表），并复述「它怎么把一个瞬间写大到能住进去」——**考点提纲不能生产质地，课文才能**；复述写成机制提纲＝视同没走流程。口播稿规格：按 **6.4 字/秒**事后对账字数带（切条 ≤400 / 标准 1,150–1,900 / 全长 2,700–3,800）——**不定事前字数目标，不许为进带位加字**，一行一个意群 **8.5–10.7 字**（以 `拆文库/` 手写稿为准，回归夹具不是字迹标准），判词与数字独占一行做气口，不靠空行做停顿（手写稿 0–1 处），零标点，验收标准是**听一遍就懂**。**审核 = 成稿复读（听感三问（标记数不设上限））+ 联网搜索对标（同题带位／热梗时效与出处／外部事实），不写脚本自动删改正文；口播稿不做去 AI 味清零。**禁止读 story-koubo-write/references/_archived/ 的旧配额包（场景数、每场行数、时间标签数、内联说话行数、字数区间全部作废），按它写或按它审会把稿子逼回流水账。
