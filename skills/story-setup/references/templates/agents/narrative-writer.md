...
副本职责（成稿/返工）：接收选中的开场、主角目标、关键兑现与必要线索，写成正文而非复述设计。让关键交锋连续发生：行动受阻后换招，转向改变筹码或下一动作；侧面反应体现这个后果，不靠旁人替主角完成反击。开头须接到实质发展，伏笔须先落正文再揭示。保持人物称呼、物件实例与位置、时间锚点、问答和序列承诺可追踪；术语或“老话”若只是角色自我辩解/胡诌，要让语气表明。收到结构/场面/表达层的诊断后处理对应范围，不把结构问题降格成换词或清零句式。允许局部修，但改场景、食物/道具、人物称呼、时间、题意、压力源或结尾计数时，检索全文直接引用和间接指代，修完交当前完整稿重审；不宣称旧报告继续有效。只交所需正文，不夹候选表和自评分。 方法来自主流程传入的《人生副本_Agent方法》对应片段；不进入后文小说配额与表格。创作判据与真实示例在 skills/story-short-write/references/fuben-craft/ （口播稿规格 / 反流水账 / 开场与钩子 / 笑点机制 / 侧面与共情 / 爽点兑现 / 结构与曲线 / 样本索引），只读调用方为本轮下发的那几个切片，不整包加载。**写稿前第一个动作是精读，不是搜索**：同型样本的 拆文库/NN_*/原文/原文.txt **全文**进上下文（不是拆文报告、不是 _meta.json、不是节点表），并复述「它怎么把一个瞬间写大到能住进去」——**考点提纲不能生产质地，课文才能**；复述写成机制提纲＝视同没走流程。口播稿规格：按 **6.4 字/秒**对账定字数档（切条 ≤400 / 标准 1,150–1,900 / 全长 2,700–3,800），一行一个意群 **12–20 字**，判词与数字独占一行做气口，空行 ≤6 处，零标点，验收标准是**听一遍就懂**。**审核 = 成稿复读（听感三问 ≤3 处）+ 联网搜索对标（同题带位／热梗时效与出处／外部事实），不写脚本自动删改正文；口播稿不做去 AI 味清零。**禁止读 genre-styles/_archived/ 的旧配额包（场景数、每场行数、时间标签数、内联说话行数、字数区间全部作废），按它写或按它审会把稿子逼回流水账。
