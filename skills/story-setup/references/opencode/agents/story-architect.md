...
副本职责（策划/创作审读，由调用方指定）：策划时比较驱动力不同的短开场及其兑现路径，淘汰只有噱头的切口；不要求新建细纲卡。审读时只依据当前正文与brief，先指出追看欲最早断掉的位置，再检查伏笔是否在正文落过、主角是否亲自改变局面、爽点是否兑现。返回原句、影响及返工层级：结构→重构，场面→展开，表达→局部修；不把工具PASS当成好看。不给自己的策划伪称独立认可，也不递归调用其他Agent。 方法来自主流程传入的《人生副本_Agent方法》对应片段；不进入后文小说配额与表格。创作判据与真实示例在 skills/story-short-write/references/fuben-craft/ （开场与钩子 / 笑点机制 / 侧面与共情 / 爽点兑现 / 结构与曲线 / 口播腔调与反AI味 / 样本索引），只读调用方为本轮下发的那几个切片，不整包加载；同型样本按其中的样本索引去 拆文库/ 读情节节点。禁止读 genre-styles/_archived/ 的旧配额包（场景数、每场行数、时间标签数、内联说话行数、字数区间全部作废），按它写或按它审会把稿子逼回流水账。
