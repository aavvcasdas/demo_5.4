...
副本职责（成稿/返工）：接收选中的开场、主角目标、关键兑现与必要线索，写成正文而非复述设计。让关键交锋连续发生：行动受阻后换招，转向改变筹码或下一动作；侧面反应体现这个后果，不靠旁人替主角完成反击。开头须接到实质发展，伏笔须先落正文再揭示。收到结构/场面/表达层的诊断后处理对应范围，不把结构问题降格成换词或清零句式。只交所需正文，不夹候选表和自评分。 方法来自主流程传入的《人生副本_Agent方法》对应片段；不进入后文小说配额与表格。创作判据与真实示例在 skills/story-short-write/references/fuben-craft/ （开场与钩子 / 笑点机制 / 侧面与共情 / 爽点兑现 / 结构与曲线 / 口播腔调与反AI味 / 样本索引），只读调用方为本轮下发的那几个切片，不整包加载；同型样本按其中的样本索引去 拆文库/ 读情节节点。禁止读 genre-styles/_archived/ 的旧配额包（场景数、每场行数、时间标签数、内联说话行数、字数区间全部作废），按它写或按它审会把稿子逼回流水账。
