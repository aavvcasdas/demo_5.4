...
| [references/genre-styles/](genre-styles/) | **定方向后必读**：按题材加载对应风格包（抖音口播专项 `人生副本实录.md`——「今天体验的人生副本是」开场的稿件优先于世情打脸 / 追妻火葬场 / 世情打脸 / 复仇打脸 / 总裁豪门 / 宅斗宫斗 / 民俗怪谈 / 悬疑 / 甜宠 / 双男主 / 沙雕脑洞），正文风格随之切换 |
| [references/fuben-craft/](fuben-craft/README.md) | **人生副本专用手艺层（profile v14 起按阶段加载）**：开场与钩子 / 笑点机制 / 侧面与共情 / 爽点兑现 / 结构与曲线 / 口播腔调与反AI味 / 样本索引。示例全部出自 `拆文库/` 语料或本仓失败稿。普通短篇网文不加载 |
...
| 人生副本（口播） | **`references/genre-styles/人生副本实录.md`（v14 入口 + 加载表）** | `references/fuben-craft/`（七个手艺文件）· `references/genre-styles/人生副本_Agent方法.md`（角色分工与切片下发）· `skills/fuben-review/SKILL.md`（审核判据）· `scripts/fuben_craft_scan.py`（只读读感扫描）· `拆文库/_速拆卡.md`（样本速查） |
