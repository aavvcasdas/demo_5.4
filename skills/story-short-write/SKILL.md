...
version: 1.4.0
description: "短篇网文写作 + 人生副本口播稿。两种稿件规格完全不同，先判类型再进流程：①口播稿（人生副本／剧本人生／第二人称人生体验／抖音口播／「今天体验的人生副本是」开场）走口播稿 profile v15，按时长与 6.4 字/秒定字数、一行一个意群、一遍听懂，不套小说的章节/付费点/去味配额；②普通短篇小说走网文流程。触发方式：/story-short-write、/写短篇、/写口播、/人生副本、「帮我写一篇短篇」「写个盐言故事」「写人生副本」「写口播稿」「剧本人生口播」「体验某种人生」。"
...
用户写的是「人生副本 / 剧本人生 / 第二人称人生体验 / 抖音口播」，或项目 profile=fuben 时，**立即进入** [人生副本口播稿 profile v15](references/genre-styles/人生副本实录.md)。**这是口播稿，不是短篇小说**：按时长与 6.4 字/秒定字数（全长档 2,700–3,800 字），一行一个意群（12–20 字），判词与数字独占一行做气口，空行 ≤6 处，零标点，验收标准是「听一遍就懂」。

**写前第一个动作是精读，不是搜索**：按 [样本索引](references/fuben-craft/样本索引.md) 选 2–3 篇同型语料，把其中至少 1 篇的 `拆文库/NN_*/原文/原文.txt` **全文**读进上下文，并复述「它怎么把一个瞬间写大到能住进去」——**考点提纲不能生产质地，课文才能**；复述写成机制提纲＝视同没走流程。

再按 profile 的**加载表**分阶段读 [fuben-craft 手艺层](references/fuben-craft/README.md)（策划与定篇幅档→结构与曲线＋样本索引＋[口播稿规格](references/fuben-craft/口播稿规格.md)；开场→开场与钩子；搞笑→笑点机制；扎心/温情→侧面与共情；爽向→爽点兑现；排版/返工/审稿→[口播排版与反流水账](references/fuben-craft/口播排版与反流水账.md)），并按 [副本 Agent 方法](references/genre-styles/人生副本_Agent方法.md) 分工：主控比较可兑现的切口 → 主笔写连续关键戏 → 审读定位追看断点 → 按结构／场面／表达分层返工（每个角色只拿本轮需要的手艺切片，见该文件 §2 的下发表）。

**审核 = 生成完再次查看 + 搜索网页**，不是写脚本删改文章：前半完整复读（听感三问 ≤3 处 + 口播规格核对），后半联网查同题爆款带位、热梗时效与出处、外部事实。**口播稿不做「去 AI 味」清零**（`story-deslop` 已把 profile=fuben 分流出去），方言俚语、答非所问的玩笑、怪数字、矛盾情绪一律保留。

机检（`scripts/fuben_run.py`）只查日期、算术、事实锁与契约，**不查钩子、笑点、共情、侧面、剧情**，PASS 不是创作批准。**禁止加载 `references/genre-styles/_archived/`**（旧配额包，按它写与审会把稿子逼回流水账）。这个分支到此结束，不继续加载下方普通小说 Reference Gate、付费点、十二列表格、默认篇幅、角色配额或去味清零表。
