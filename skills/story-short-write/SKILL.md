...
version: 1.5.0
description: "短篇网文写作。辅助短篇小说创作，从构思到成稿，聚焦情绪拉扯与节奏把控。**人生副本／剧本人生／第二人称人生体验／抖音口播稿不在本 Skill，转 `story-koubo-write`（/写口播、/人生副本）**；识别到这类意图或 profile=fuben 时立即转出，不加载本文件的 Reference Gate、付费点、表格、篇幅与去味配额。触发方式：/story-short-write、/写短篇、「帮我写一篇短篇」「写个盐言故事」。"
...
## 先分流：人生副本口播稿已迁出，转 story-koubo-write

先识别任务动作：用户只要求研究/借鉴/改造Agent时，不启动下述写稿流程，不用自写样稿替代借鉴。

**人生副本 / 剧本人生 / 第二人称人生体验 / 抖音口播稿已迁出本 Skill。**
它们是口播稿，规格与短篇小说完全不同（按秒计不按章计、一行一个意群、验收标准是听一遍就懂），
现在由独立的 **[`story-koubo-write`](../story-koubo-write/SKILL.md)** 负责：
`/写口播`、`/人生副本`、`「写人生副本」`、`「体验某种人生」`、`「今天体验的人生副本是」` 都路由到它。

识别到上述意图或项目 `profile=fuben` 时，**立即转 `story-koubo-write`，本文件下方内容一律不加载**——
不加载 Reference Gate、付费点、十二列表格、默认篇幅、角色配额或去味清零表。
副本的创作判据、口播稿规格、手艺层与旧配额包（`_archived/`，禁止加载）都在新 Skill 的 `references/` 下。
审稿转 [`fuben-review`](../fuben-review/SKILL.md)：**审核 = 成稿复读 + 联网搜索对标**，不跑脚本删改正文，
口播稿不做「去 AI 味」清零（`story-deslop` 同样已把 profile=fuben 分流出去）。

新项目仍可在**作品目录**放 `.fuben.json`：`{"schema_version":1,"profile":"fuben"}`，供工具与 hooks 识别；
这不是另一份创作表，未启用 hooks 的直接创作不要求它。普通小说分支保持原合同。
