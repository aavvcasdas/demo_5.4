...
// BEGIN FUBEN PROFILE (canonical: story-review/scripts/story-profile.js)
...
function proseBlockReason(root, absolute) {
  ...
  const proseTarget = /^正文(?:_[^/]*)?\.md$/.test(base) || (parent === "正文" && /^第.*章.*\.md$/.test(base))
  if (proseTarget && isFubenProject(absolute)) return null
  if (base === "正文.md") {
...
function isProsePath(absolute) {
  const base = path.basename(absolute)
  const parent = path.basename(path.dirname(absolute))
  if (/^正文(?:_[^/]*)?\.md$/.test(base) && isFubenProject(absolute)) return true
...
function proseAfterWrite(root, absolute) {
  ...
  if (isFubenProject(absolute)) {
    const text = fs.readFileSync(absolute, "utf8")
    if (!text.trim()) return "ERROR 人生副本正文为空；不能进入审核。"
    return "NOTE 人生副本走轻量 profile；通用小说句式/段长/末句标点网不适用。请跑 fuben_run.py，再按证据复核；此提示不代表已审核。"
  }
  const findings = []
  try {
